...
def saveResult(request,saveResultid_from_url):
	trans = transationRecord.objects.get(id=saveResultid_from_url)
	...
	if len(checkRe)==0:
		if request.GET['consignmentNo'] and request.GET['is_pass'] and request.GET['reason']:
			...
			if isp=='y':
				...
				tdate = tra.tdate
				customerNumber = tra.customerNumber
				transa = transationRecord(id=tid,tdealer=tdealer,tState='Refuse',tID=tID,tAccount=tAccount,consignmentNo=con,transactionNo=transactionNo,transationAmount=transationAmount,transationPrice=transationPrice,tdate=tdate,customerNumber=customerNumber)
				transa.save()
	if len(checkRe)==1:
		if request.GET['consignmentNo'] and request.GET['is_pass'] and request.GET['reason']:
			con = request.GET['consignmentNo']
			isp = request.GET['is_pass']
			res = request.GET['reason']
			checkr = checkResult(id=checkRe[0].id,consignmentNo=con,sName=request.session["sName"],is_pass=isp,checkInformation=res)
			checkr.save()
			if isp=='y':
				tra = transationRecord.objects.get(id=saveResultid_from_url)
				tid = tra.id
...
def addTrades(request):
	...
	if request.GET['tID'] and request.GET['tAccount'] and request.GET['consignmentNo'] and request.GET['transactionNo'] and request.GET['transationAmount'] and request.GET['transationPrice'] and request.GET['tdate'] and request.GET['customerNumber']:
		...
		rec = transationRecord(tdealer=request.session["dName"],tState='undistribute',tID=tid, tAccount=tac, consignmentNo=con, transactionNo=tra, transationAmount=tam, transationPrice=tpr, tdate=tda, customerNumber=cus)
		rec.save()
...
def editSave(request,saveid_from_url):
	if request.GET['tID'] and request.GET['tAccount'] and request.GET['consignmentNo'] and request.GET['transactionNo'] and request.GET['transationAmount'] and request.GET['transationPrice'] and request.GET['tdate'] and request.GET['customerNumber']:
		...
		tda = request.GET['tdate']
		cus = request.GET['customerNumber']
		rec = transationRecord(id=saveid_from_url,tdealer=request.session["dName"],tState='undistribute',tID=tid, tAccount=tac, consignmentNo=con, transactionNo=tra, transationAmount=tam, transationPrice=tpr, tdate=tda, customerNumber=cus)
		rec.save()
		return HttpResponseRedirect('/index/')
...
def supervisorpropertyInfo(request,supervisortID_from_url):
	ass = assetInformation.objects.get(tID=supervisortID_from_url)
	return render_to_response('supervisorpropertyInfo.html',locals())
